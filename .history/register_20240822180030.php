...
<body>
  ...
  <a href="http://localhost/book%20management/verify.php?token='.$code.'" style="background-color: #4CAF50; color: white; padding: 15px 25px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer;">Verify Email</a>
</body>
</html>';
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirmPassword)) {
       ...
       if (mysqli_num_rows($stmt)> 0) {
        ...
        $_SESSION['status'] = "<div class='alert alert-danger'>Email already exists.</div>";
            ...
            $query = mysqli_query($conn,"INSERT INTO users (email,username,password,confirm,code) VALUES ('$email','$username','$hashedPassword','$hashedconfirm','$code')");
    
            if ($query){
                sendEmail($user,$email,$code);
                $_SESSION['status'] = "<div class='alert alert-success'>Registration successful please verify you Email. You can now <a href='login.php'>login</a>.</div>";
            header("location: register.php");
                exit(0);
            }else{
               
                $_SESSION['status']= "<div class='alert alert-danger'>Registation is failed</div>";
                header("location: register.php");
                exit(0);
            }
        }
