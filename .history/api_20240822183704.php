...
switch ($action) {
    ...
    case 'add_book':
        ...
        if ($title && $author_id && $category_id) {
            $query = $conn->prepare("INSERT INTO books (title, author_id, category_id) VALUES ('$title',$author_id, $category_id)");
    ...
    case 'update_book':
        ...
        if ($id && $title && $author_id && $category_id) {
            $query = $conn->prepare("UPDATE books SET title ='$title', author_id =$author_id, category_id =$category_id WHERE id =$id");
            if ($query) {
